...
##
## A propellant tank used to masquerade as its own rigid body. In the single-body
## vessel model, that would create a second physics authority and a joint where
## the ship only needs a mass ledger. `PropellantModel` is now the tank component
## that preserves the fuel story without splitting the craft into separate
## dynamic bodies.
##
## As a `MassModel`, the tank contributes dry container mass and remaining
## propellant mass to `VesselRigidBody3D`. In the authored tank scene, the script
## is attached to a `CollisionShape3D` root, which lets the same component supply
## its physical outline to the compound rigid body. Engines draw fuel through
## `consume_propellant(...)`; the tank reports the actual amount burned and uses
## the inherited mass notification path to refresh total mass and center of mass.
##
